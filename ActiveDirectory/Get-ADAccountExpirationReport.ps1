<#
.SYNOPSIS
    Get-ADAccountExpirationReport.ps1 - Active Directory Management Tool
...
[CmdletBinding()]
param(
    [string]$OutputPath = "$env:USERPROFILE\Documents\Get-ADAccountExpirationReport_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
    [string]$SearchBase,
    [int]$InactiveDays = 90
)

$ScriptName = "Get-ADAccountExpirationReport"
...
Write-Log "========== Get-ADAccountExpirationReport Started ==========" -Level "INFO"
...
Write-Log "========== Get-ADAccountExpirationReport Completed ==========" -Level "SUCCESS"
