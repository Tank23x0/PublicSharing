<#
.SYNOPSIS
    Get-ADGroupMembershipReport.ps1 - Active Directory Management Tool
...
[CmdletBinding()]
param(
    [string]$OutputPath = "$env:USERPROFILE\Documents\Get-ADGroupMembershipReport_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
    [string]$SearchBase,
    [int]$InactiveDays = 90
)

$ScriptName = "Get-ADGroupMembershipReport"
...
Write-Log "========== Get-ADGroupMembershipReport Started ==========" -Level "INFO"
...
Write-Log "========== Get-ADGroupMembershipReport Completed ==========" -Level "SUCCESS"
